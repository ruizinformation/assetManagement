<template>
  <el-dialog :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
      ...
      <el-form-item label="上级菜单" prop="parentName" v-if="dataForm.type > 0">
        <el-popover ref="menuListPopover" placement="bottom-start" trigger="click">
          <el-tree :data="menuList" :props="menuListTreeProps" node-key="id" ref="menuListTree" @current-change="menuListTreeCurrentChangeHandle"
            :default-expand-all="false" :highlight-current="true" :expand-on-click-node="false">
          </el-tree>
        </el-popover>
    ...
    init (id) {
      ...
      this.$http({
        ...
        if (!this.dataForm.id) {
          ...
          // 修改
          this.$http({
            url: this.$http.adornUrl(`/sys/menu/${this.dataForm.id}`),
            method: 'get',
            params: this.$http.adornParams()
          }).then(({ data }) => {
